...
|---|---|---|---|---|
| Creation-time | `data/raw/dataset_at_creation.csv` | 13,895 | 36 | Features known at task assignment |
| Creation-time (fixed target) | `data/fixed_end_date/dataset_at_creation_fixed_end_date.csv` | 13,895 | 29 | Same but with three-tier target (47.8% overdue) |
| Halfway | `data/raw/dataset_at_halfway.csv` | 13,895 | 51 | Creation + 15 accumulation features (available at midpoint) |
| Halfway (fixed target) | `data/fixed_end_date/dataset_at_halfway_fixed_end_date.csv` | 13,895 | 44 | Same but with three-tier target (47.8% overdue) |

**Leak-fixed versions** (all features are cutoff-date-aware, 8 challenge features dropped, `position_id_encoded` excluded):

| Variant | File | Rows | Cols | Purpose |
|---|---|---|---|---|
| Creation-time (clean) | `data/clean/dataset_at_creation_clean.csv` | 13,895 | 29 | Leak-free features known at task creation |
| Halfway (clean) | `data/clean/dataset_at_halfway_clean.csv` | 13,895 | 40 | Leak-free features at task midpoint |
...
Pipeline source (leaky): [`src/build_features.py`](../src/build_features.py)  
Leak-fixed pipeline: [`src/build_features_leak_fixed.py`](../src/build_features_leak_fixed.py)  
Reusable feature functions: [`src/feature_engineering.py`](../src/feature_engineering.py)  
ETL SQL (leaky): [`sql/analytical_dataset.sql`](../sql/analytical_dataset.sql)  
Leak-fixed SQL: [`sql/analytical_dataset_leak_fixed.sql`](../sql/analytical_dataset_leak_fixed.sql)  
Data quality SQL: [`sql/data_quality_checks.sql`](../sql/data_quality_checks.sql)  
Relationship docs: [`handoff_H2.md`](handoff_H2.md)  
Data quality docs: [`handoff_H5.md`](handoff_H5.md)  
Schema docs: [`handoff_H1.md`](handoff_H1.md)
...
Source: [`handoff_H5.md`](handoff_H5.md), [`sql/data_quality_checks.sql`](../sql/data_quality_checks.sql)
...
- [`handoff_H1.md`](handoff_H1.md) — dataset schema & data sources (full feature inventory with types)
- [`handoff_H2.md`](handoff_H2.md) — table relationships & join strategy (PK/FK matrix, resolution cascades)
- [`handoff_H5.md`](handoff_H5.md) — data quality findings (null rates, sparsity, date integrity)
- [`src/build_features.py`](../src/build_features.py) — end-to-end pipeline (leaky version)
- [`src/build_features_leak_fixed.py`](../src/build_features_leak_fixed.py) — leak-free pipeline
- [`src/feature_engineering.py`](../src/feature_engineering.py) — reusable feature functions
- [`sql/analytical_dataset.sql`](../sql/analytical_dataset.sql) — full extraction SQL (leaky version)
- [`sql/analytical_dataset_leak_fixed.sql`](../sql/analytical_dataset_leak_fixed.sql) — leak-free SQL reference
- [`sql/data_quality_checks.sql`](../sql/data_quality_checks.sql) — 15+ validation checks
